...
import type { SubjectStatus } from '@pensumtrack/types'
...
export default function PensumPage() {
  ...
  const handleToggleStatus = async (code: string, current: SubjectStatus) => {
    const next = current === 'passed' ? 'PENDING'
               : current === 'in-progress' ? 'PASSED'
               : current === 'available' || current === 'pending' ? 'IN_PROGRESS'
               : null
    if (!next) return
          ...
          <div key={sem}>
            ...
            <div className="flex flex-col gap-2">
              {subjects.map((s) => {
                ...
                return (
                  <button key={s.code} onClick={() => handleToggleStatus(s.code, status)}
                          className="flex items-center justify-between p-3 rounded-xl text-left w-full transition-all"
                          style={{ background: 'var(--surface)', border: `1px solid var(--pt-border)` }}>
